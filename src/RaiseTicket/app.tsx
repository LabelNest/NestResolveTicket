...
const handleCreateTicket = async (formData: Record<string, any>) => {
  // Use the state we set in useEffect
  if (!authUser) {
  ...
  if (!tenantId) {
    console.error("tenant_id missing in user metadata");
    alert("Configuration Error: Missing Tenant ID");
    return;
  }
  ...
  const newTicketData = {
    ...
    tenant_id: tenantId,        // ✅ Correct: Use the Tenant ID from metadata
    created_by: authUser.id,    // ✅ Correct: Use the specific User ID
  };
...
const handleCreateTicket = async (formData: Record<string, any>) => {
  ...
  // STEP 2: Get tenant_id from JWT metadata
  const tenantId = user.user_metadata?.tenant_id;
